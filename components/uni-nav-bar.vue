<template>
	<view class="uni-navbar" :class="{'uni-navbar-fixed':isFixed,'uni-navbar-shadow':hasShadow}" :style="{'background-color':backgroundColor}">
		...
		<view class="uni-navbar-header" :style="{color:color}">
			<view class="uni-navbar-header-btns" @tap="onClickLeft" v-if="isShowleft">
			...
			<view class="uni-navbar-header-btns" @tap="onClickRight">
                <view v-if="rightIcon.type.length">
                    <uni-icon :family="rightIcon.family" :type="rightIcon.type" :color="rightIcon.color || color" :size="rightIcon.size"></uni-icon>
                </view>
                <!-- 优先显示图标 -->
                <view v-if="rightText.length&&!rightIcon.type.length" class="uni-navbar-btn-text">{{rightText}}</view>
